...
CONCLUSION: **D2e/D4c delivered on both counts.** Truncation is retired (0.3754 → 0.0073, and 0.0000 once capped generations are errors), which un-confounds every length-increasing injector in the C4 matrix; and ID loss is now a first-class run-level metric that caught a live 2.5% omission on its first real use. The cost is −0.65 chrF on hadith. The block-level MetricX detection matrix is running detached (6,470 segments; log `metricx-blocks.log`, sentinel `done-metricx-blocks`, ETA ~10h) — it will say whether short blocks also move the *detection* rates, which this entry does not claim. ⚠️ Unexpected cost fact measured while launching it: **MetricX CPU time is dominated by fixed per-segment overhead, not input length** — 5.5 s/segment on 394-token block inputs vs 7.2 s/segment on 1,085-token item inputs. Shortening inputs 2.8× bought 24% per segment while multiplying segment count by 2.8×, so blocks make QE *more accurate and more expensive at once* (~10h vs 4.6h). That reverses the intuition the block design was chosen on and is the concrete argument for moving QE to Modal before the benchmark grows.
