...
- **`ssh wayway`** (Webuzo/Apache VPS; connection details in local ssh config): sites under `/home/bilal/public_html/<domain>/`. **Serving quirk:** only `versed.wayway.ai` actually resolves to this box — apex/`www.wayway.ai` HTTPS is served by a *different origin*, so the `wayway.ai` docroot is unreachable publicly. The `versed.wayway.ai` vhost serves static files from the `versed` docroot (that's where `/audio/openiti-previews/...` and the lab dashboard at `/translator/` live). Limited disk on `/` — do not stage corpora there.
