...
**Block-level matrix IN FLIGHT (launched 2026-08-14 20:41, ETA ~10h):** 3,235 pairs / 6,470 segments over the 522-block TG12B run (`~/versed-translator-data/qe/tg12b-blocks-metricx/`; log `metricx-blocks.log`, sentinel `done-metricx-blocks` — absent = running, `0` = success). 14/15 injectors now exercised (`collapse_paragraphs` fires on block output where it could not before; only `alter_citation` remains unexercised on this slice). **Verify `score_min`/`score_max`/`distinct_scores` before trusting any rate** — and note the denominator changed (6,470 vs 2,288 segments, shorter each), so compare *per-injector rates*, never raw counts.

⚠️ **MetricX CPU cost is dominated by fixed per-segment overhead, not input length** — measured here, and it is a planning fact worth having: 5.5 s/segment on 394-token block inputs vs 7.2 s/segment on 1,085-token item inputs. Cutting input length by 2.8× bought ~24% per segment while multiplying segment *count* by 2.8×, so the block-level study costs ~10h against the item-level study's 4.6h. **Blocks make QE more accurate and more expensive at the same time.** Any QE run over a genre-diverse (larger) benchmark must therefore go to Modal — the existing standing rule now has a number behind it.
