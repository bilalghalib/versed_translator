...
**Block-level matrix IN FLIGHT (launched 2026-08-14 20:41, ETA ~9.7h):** 3,235 pairs / 6,470 segments over the 522-block TG12B run (`~/versed-translator-data/qe/tg12b-blocks-metricx/`; log `metricx-blocks.log`, sentinel `done-metricx-blocks` — absent = running, `0` = success). 14/15 injectors now exercised (`collapse_paragraphs` fires on block output where it could not before; only `alter_citation` remains unexercised on this slice). **Verify `score_min`/`score_max`/`distinct_scores` before trusting any rate** — and note the denominator changed (6,470 vs 2,288 segments, shorter each), so compare *per-injector rates*, never raw counts.

⚠️ **MetricX CPU cost scales sub-linearly with input length, so blocks cost ~2× more in total.** Compared gross (wall clock ÷ segments, the figure that actually predicts an ETA) so both sides are measured the same way: **5.4 s/segment on 394-token block inputs vs 7.24 s/segment on 1,085-token item inputs.** A 2.83× shorter input buys only **26%** less time per segment — fixed per-segment overhead dominates — while blocking multiplies segment *count* by 2.83×. Net **2.1×**: ~9.7h against the item-level study's 4.6h. **Blocks make QE more accurate and about twice as expensive at the same time**, the opposite of the intuition the design was chosen on. Any QE run over a genre-diverse (larger) benchmark must therefore go to Modal; the standing rule now has a number behind it. (Two earlier estimates in this session, 5.5 and 4.2 s/segment, were computed over short windows and were noise; the gross rate above is stable and is the one to use.)
