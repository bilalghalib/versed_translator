...
SUBPACKAGES = ["benchmark", "harness", "qe", "align", "corpus", "throughput"]
...
def test_cli_entry_functions_run(capsys):
    from versed_translator import cli

    for fn in (cli.benchmark_main, cli.harness_main, cli.corpus_main):
        rc = fn()
        assert rc == 0
