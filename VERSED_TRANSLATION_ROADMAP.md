...
**Block-level matrix IN FLIGHT (launched 2026-08-14 20:41, ETA ~7.5h):** 3,235 pairs / 6,470 segments over the 522-block TG12B run (`~/versed-translator-data/qe/tg12b-blocks-metricx/`; log `metricx-blocks.log`, sentinel `done-metricx-blocks` — absent = running, `0` = success). 14/15 injectors now exercised (`collapse_paragraphs` fires on block output where it could not before; only `alter_citation` remains unexercised on this slice). **Verify `score_min`/`score_max`/`distinct_scores` before trusting any rate** — and note the denominator changed (6,470 vs 2,288 segments, shorter each), so compare *per-injector rates*, never raw counts.

⚠️ **MetricX CPU cost scales sub-linearly with input length, so blocks cost more in total.** Measured in flight over the first 128 segments (two points, so treat as an estimate until the sentinel lands): **4.2 s/segment on 394-token block inputs vs 7.2 s/segment on 1,085-token item inputs** — a 2.8× shorter input buys only 1.7× less time per segment, because fixed per-segment overhead dominates. But blocking multiplies segment *count* by 2.8×, so the block-level study runs ~7.5h against the item-level study's 4.6h. **Blocks make QE more accurate and ~1.6× more expensive at the same time** — the opposite of the intuition the design was chosen on. Any QE run over a genre-diverse (larger) benchmark must therefore go to Modal; the standing rule now has a number behind it.
